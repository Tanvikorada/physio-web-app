...
function SessionPageContent({ 
  ...
  // Pre-session briefing no longer blocks the flow, but we can still fetch if needed later
  React.useEffect(() => {
    // ...
  ...
  const handlePrePainSubmit = async (score: number) => {
    ...
    // Mode D: skip camera — go straight to guided timer
    if (initialExerciseData?.trackingMode === "D") {
      setSessionState("guided")
      return
    }

    // All other modes: go straight to camera
  ...
  if (initialExerciseData?.landmarkConfig) {
    let top = initialExerciseData.landmarkConfig.rep_top_angle
    const start = initialExerciseData.landmarkConfig.rep_start_angle
    const isInverted = initialExerciseData.landmarkConfig.angle_calculation === "180-raw" || start > top
    if (isInverted && start > top) {
      top = 180 - top
    }
    baseTargetAngle = top
  } else {
    baseTargetAngle = exercise === "KneeFlexion" ? 135 : 180
  }
